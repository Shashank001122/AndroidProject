...
app.get("/GuardianNine", function (req, res) {
    //var val = req.query.val
    result = {}
    //request("https://content.guardianapis.com/search?order-by=newest&show-fields=starRating,headline,thumbnail,short-url&api-key=2ab573c1-5426-4d28-96c4-bf13d2a166b5")
    request('https://content.guardianapis.com/search?order-by=newest&show-fields=starRating,headline,thumbnail,short-url&api-key=2ab573c1-5426-4d28-96c4-bf13d2a166b5', { json: true }, (err, response, body) => {
        if (err) { return console.log(err); }
        res.send(body)
    });
});
...
///Science////
app.get("/GuardianSci", function (req, res) {
    //var val = req.query.val
    result = {}
    request('https://content.guardianapis.com/science?api-key=2ab573c1-5426-4d28-96c4-bf13d2a166b5&show-blocks=all', { json: true }, (err, response, body) => {
        if (err) { return console.log(err); }
        res.send(body)
    });
});

app.get('/trends/:keywords', function (req, res) {
    keyword = req.params.keywords;
    console.log(keyword)
    gettrendsData(keyword, function (response) {
        // console.log(response);
        var obj = JSON.parse(response)
        var timeline = obj.default.timelineData;
        result = []
        for (var i = 0; i < timeline.length; i++) {
            result.push((i, timeline[i].value[0]))
        }
        dict = {}
        dict['res'] = result
        res.send(dict)
...
function gettrendsData(url, callback) {
    let dat1 = new Date(2019, 06, 01);
    googleTrends.interestOverTime({ keyword: url, startTime: new Date(2019, 06, 01) })
...
app.get("/weatherfind", function (req, res) {
    var val1 = req.query.appid
    var val2 = req.query.lon
    var val3 = req.query.lat
    console.log(val1)
    console.log(val2)
    console.log(val3)
    result = {}
    url = "http://api.openweathermap.org/data/2.5/weather?appid=" + val1 + "&lon=" + val2 + "&lat=" + val3
